...
build=${cuvina}/build_gpu
mkdir -p $build
cd $build
#cmake -DDBG_BFGS_USE_CPU=OFF -DCMAKE_BUILD_TYPE=Debug ${cuvina}
cmake -DDBG_BFGS_USE_CPU=OFF -DCMAKE_BUILD_TYPE=Release ${cuvina}
make vina -j 16
mkdir -p ${cuvina}/test
cd ${cuvina}/test
...
$build/vina --gpu ${device} --receptor $cuvina/example/python_scripting/1iep_receptor.pdbqt \
		...
		--seed 123 \
		--verbosity 2 \
		--exhaustiveness $concurrent 2>&1 | tee ${curr}/gpu.log
